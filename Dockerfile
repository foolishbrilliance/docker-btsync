...
RUN echo 'while true; do pidof btsync && echo -e "HTTP/1.1 200 OK\r\nDate: $(date)\r\n\r\nOK: btsync is running as of $(date) on $(cat /tmp/instance-id):$(cat /tmp/public-ipv4)" | nc -l 8080 || echo -e "HTTP/1.1 500 INTERNAL SERVER ERROR\r\nDate: $(date)\r\n\r\nERROR: btsync is NOT running as of $(date) on $(cat /tmp/instance-id):$(cat /tmp/public-ipv4)" | nc -l 8080; done &' >> /tmp/init.sh
# Run btsync
